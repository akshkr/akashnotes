In this section, we will cover some fundamental terminologies that will be used throughout our learning. If you are already familiar with these terms, feel free to skip this section.
...
Let us look at an example of hypothsis function. Say, we are predicting house price using area and number of floors in a house.

$$ HOUSE\_PRICE  (\hat{y}) = \theta_{0} + (\theta_{1} * AREA) +  (\theta_{1} * NUM\_OF\_FLOOR) $$

Now lets say that after training we got the values of theta as:

$$ HOUSE\_PRICE  (\hat{y}) = \$10K + (\$1K * AREA) +  (\$100K * NUM\_OF\_FLOOR) $$

Now we can feed values of area and number of floor to get the price of house:

For area 2000 sq ft and 2 floors, the price of house will be -
$$\$10K + (\$100 * 2000) +  (\$100K * 2) = \$410K$$
...
That was a lot of information to take in! Some concepts may be clear to you while others, such as over-fitting and cross-validation, may still be confusing. Don't worry, we'll cover these concepts in more detail when we have context for modeling. For now, let's move on to our first machine learning algorithm.